...
data_list = list(
  ...
  interaction_mask = interactionMask_vector,
  y0               = y0_meanSubjects,
...
if(scale>0){ # NEGATIVE BINOMIAL LIKELIHOOD
  # RECOMPILE EACH TIME
  if(file.exists("MODELS/MODEL_B1_INT.rds")){
    file.remove("MODELS/MODEL_B1_INT.rds")
  }
  M_model  = stan_model("MODELS/MODEL_B1_INT.stan")
  ...
  # RECOMPILE EACH TIME
  if(odeSolver==45){
    if(file.exists("MODELS/MODEL_B1_FLT_45.rds")){
      file.remove("MODELS/MODEL_B1_FLT_45.rds")
    }
    print('Picked ODE_45 model')
    M_model  = stan_model("MODELS/MODEL_B1_FLT_45.stan")
    print('Compiled the model')
  }else{
    odeSolver = 0 #WILL GIVE THIS AS INPUT BUT STILL TO MAKE SURE IT IS A NUMBER FOR THE FILENAME
    if(file.exists("MODELS/MODEL_B1_FLT_BDF.rds")){
      file.remove("MODELS/MODEL_B1_FLT_BDF.rds")
    }
    print('Picked ODE_BDF model')
    M_model  = stan_model("MODELS/MODEL_B1_FLT_BDF.stan")
    print('Compiled the model')
  }
...
mkdir(direc2save)
save(T_model, file = paste0(direc2save,"/MODEL_SMOOTH_",smoothed,"_ODESOLVER_",odeSolver,"_B1_",round(tstamp),".RData"))
