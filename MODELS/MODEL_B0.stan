functions {
  ...
  int[]  x_i 
    ...
    for(r in 1:numTaxa){
      for(c in 1:numTaxa){
        if(r==c){
          interactionMat[r,c]=-1*interactionMat_vector_diag[counter_diag];// will sample this from beta distribution so will be strictly positive
          counter_diag = counter_diag + 1;
        }else{
          interactionMat[r,c]=interactionMat_vector_nondiag[counter_nondiag];
    ...
    // print("gr: ", growthRate_vector)
    
    for(tx in 1:numTaxa){
      // print("tx: ", tx)
      growthRate_tx      = growthRate_vector[tx];
      interactionvectemp = interactionMat[tx,1:numTaxa];
      // print(interactionvectemp)
      // print(abundancevectemp)
      interactions_tx    = dot_product(interactionvectemp,abundancevectemp); // checked, this works 
      // print(interactions_tx)
      dydt[tx]           = y[tx]*(growthRate_tx + interactions_tx);
    }
...
data {
  ...
  // priors
  real p_growth_rates[2];
  real p_interaction_terms_nondiag[2];
  real p_interaction_terms_diag[2];
  real p_phi;
...
transformed parameters {
  real theta[(2*numTaxa+numTaxa*numTaxa)]; // vector of parameterss
  real<lower=0> y[numTimeSteps,numTaxa]; // raw ODE output 
  real<lower=0> output_mat[numTimeSteps,numTaxa];
  
  theta[1:numTaxa] = growthRate_vector;
  theta[(1+numTaxa):(2*numTaxa)] = interactionMat_vector_diag;
  theta[(1+2*numTaxa):(numTaxa+numTaxa*numTaxa)] = interactionMat_vector_nondiag;
  theta[(1+numTaxa+numTaxa*numTaxa):(2*numTaxa+numTaxa*numTaxa)] = phi;
  
  // print("phi: ",theta[(1+numTaxa+numTaxa*numTaxa):(2*numTaxa+numTaxa*numTaxa)])
...
model {
  // priors
  // growthRate_vector ~ beta(p_growth_rates[1],p_growth_rates[2]);
  // interactionMat_vector_diag ~ beta(p_interaction_terms_diag[1],p_interaction_terms_diag[2]);
  growthRate_vector ~ normal(p_growth_rates[1],p_growth_rates[2]);
  interactionMat_vector_diag ~ normal(p_interaction_terms_diag[1],p_interaction_terms_diag[2]);
  interactionMat_vector_nondiag ~ normal(p_interaction_terms_nondiag[1],p_interaction_terms_nondiag[2]);
  phi ~ exponential(p_phi);
  
  // print(output_mat[1,1])
