functions {
  ...
  int[]  x_i 
    ...
    int interactionMask_vector = x_i[3:(2+numTaxa*numTaxa)];
    ...
    interactionMat_vector    = theta[(1+numTaxa):(numTaxa+numTaxa*numTaxa)];
    ...
    for(r in 1:numTaxa){
      for(c in 1:numTaxa){
          interactionMat[r,c]= interactionMask_vector[counter]*scale*interactionMat_vector[counter];
          counter = counter + 1;
      }
    }
...
data {
  ...
  int interaction_mask[numTaxa,numTaxa];
...
parameters{
  ...
  real<lower=0> interactionMat_vector[numTaxa]; // all positive
}
...
transformed parameters {
  ...
  theta[(1+numTaxa):(numTaxa+numTaxa*numTaxa)] = interactionMat_vector;
...
model {
  ...
  interactionMat_vector ~ normal(p_a[1],p_a[2]);
