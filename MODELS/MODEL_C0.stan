functions {
  ...
  int[]  x_i 
    ...
    for(tx in 1:numTaxa){
      ...
      // UNCOATED
      dydt[tx]          = theta[tx]*(1-level_mIgA)*(growthRate_net * y[tx] + interactions/total_abundance);
      
      // COATED
      dydt[tx+numTaxa]  = theta[tx]*(level_mIgA)*(growthRate_net * y[tx+numTaxa] + interactions/total_abundance);
